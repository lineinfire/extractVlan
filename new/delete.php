  ...
  <section class="content">
    ...
    <form id="bghimire" method="post"> 
     <div class="container well" style="margin-top: 3.8%; width: 98%; height:auto; background-color: rgba(52,81,107,0.82); color:#c9cbd2; border-color: silver;">
        ...
        <fieldset>
          <legend><h5>Delete Role and Rights</legend></h5>
        </fieldset>
<br>
        <div class="form-group col-md-12">
          <div class="input-group">
           ...
           <select class="form-control" placeholder="Switch IP" id="switchid" name="switchid"> <option class="form-control" value="" selected>Please module Code</option><?php
            mysql_connect('localhost', 'root', '');
            mysql_select_db('multi-admin');
            $sql = "SELECT distinct `mod_modulecode` FROM `module`";
...
<h5>
  Please Note: &nbsp The roles assigned will be automatically deleted upon module deletion... <br><p><b></h5></div>
   <br>
...
<hr>
  ...
  <div class="form-group col-md-12">
    <button type="submit" class="btn btn-default" name="btn-login" id="btn-login">
      <span class="glyphicon glyphicon-log-in"></span> &nbsp; Delete Module
    </button> 
    <div id="loader"></div>
  </div> 
            ...
            jQuery.ajax({
              ...
              { 
                ...
                setTimeout(
                  function() 
                  {
                    $("#btn-login").html('</span> <img src="ajax-loader.gif" /> &nbsp; Updating the roles ...');
              ...
              success: function(response){
                ...
                if(response == "Success"){
                  setTimeout(function() {
                    ...
                    $("#btn-login").html('<span class="glyphicon glyphicon-log-in"></span> &nbsp; Assgin Roles');
                    $("#error").html('<div class="alert alert-success style="color:#eeeeee;"> <span class="glyphicon glyphicon-info-sign"></span> &nbsp; '+response+' !</div>');
                  }, 6000);
                }
                
                else if(response == "Role Updated Successfully")
                {
                  console.log(response);
                  setTimeout(function() {

                    $("#btn-login").html('<span class="glyphicon glyphicon-log-in"></span> &nbsp; Delete Module');
      ...
      $(document).ready(function(){
        ...
        if (switchid != '') {
          ...
          $.ajax({
            ...
            async:true,
            beforeSend: function () {
              $('#pageid').html('<img src="loader.gif" alt="" width="24" height="24">');
            },
  ...
  $(document).ready(function(){
    ...
    //Hide div w/id extra
     $("#extra").css("display","none");
      $("#modulegroupcode").hide();
    // Add onclick handler to checkbox w/id checkme
     $("#deletemodule").click(function(){

    // If checked
    if ($("#deletemodule").is(":checked"))
    {
      

$("#modulegroupcode").show("fast");
